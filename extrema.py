...
def Newtonian(f, n, l0, d = {}, criterion = 1.0/10**4):
	...
	while s**0.5>criterion:
		...
		for j in range(n):
			...
			for k in range(n):
				dd[j].append(DD(f, j, k, n, l, d))
		dd = np.array(dd)
		l = l-np.dot(dl, np.linalg.inv(dd))
		s = 0
		for k in range(n):
			s = D(f, k, n, l, d)**2+s
			lv[k].append(l[k])
...
def Conjugate(f, n, l0, d = {}, criterion = 1.0/10**4, a0 = 1.0/10**4):
	i = 0
	s = 0
	lv = []
	l = []
